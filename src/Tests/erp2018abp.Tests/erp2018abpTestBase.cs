...
using erp2018abp.Migrations.SeedData;
...
namespace erp2018abp.Tests
{
    public abstract class erp2018abpTestBase : AbpIntegratedTestBase<erp2018abpTestModule>
    {
        ...
        protected void UsingDbContext(Action<erp2018abpDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<erp2018abpDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<erp2018abpDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<erp2018abpDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<erp2018abpDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<erp2018abpDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<erp2018abpDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<erp2018abpDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<erp2018abpDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<erp2018abpDbContext, Task<T>> func)
        {
            T result;
